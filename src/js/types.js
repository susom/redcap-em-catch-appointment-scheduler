...
jQuery(document).on('click', '.time-slot', function (e) {
     ...
     */
    if (jQuery(this).data('show-department') == "0") {
        jQuery("#show-department").hide();
    } else {
        jQuery("#show-department").show();
    }

    /**
     * do we need to location options section based on config.json
     */
    if (jQuery(this).data('show-supervisor') == "0") {
        jQuery("#show-supervisor").hide();
    } else {
        jQuery("#show-supervisor").show();
    }
